...
import com.freebrio.robustdemo.util.DeviceUtils
import java.io.File
...
fun getPatchName(): String? {
    return SharePrefUtils.getString(MyApplication.getContext(), PREF_NAME, KEY_PATCH_NAME, null)
}

fun setPatchName(patchVersion: String?) {
    SharePrefUtils.putString(
        MyApplication.getContext(), PREF_NAME,
        KEY_PATCH_NAME, DeviceUtils.getAppVersionCode() + "_" + patchVersion
    )
}

fun getPatchHash(): String? {
    return SharePrefUtils.getString(MyApplication.getContext(), PREF_NAME, KEY_PATCH_HASH, null)
}

fun setPatchHash(patchHash: String?) {
    SharePrefUtils.putString(MyApplication.getContext(), PREF_NAME, KEY_PATCH_HASH, patchHash)
}

fun isPatchAppInit(): Boolean {
    return SharePrefUtils.getBoolean(MyApplication.getContext(), PREF_NAME, KEY_PATCH_APP_INIT, false)
}
...
fun getPatchRootDir(): String {
    val rootDirPath =
        MyApplication.getContext().filesDir.toString() + File.separator + ROOT_DIR_NAME
 ...
 */
fun clearAll() {
    SharePrefUtils.clear(MyApplication.getContext(), PREF_NAME)
}
